...
const body = [
  ...
  P('방문자 화면에서는 카테고리 탭에 사진 장수가 표시되고, "전체 / 그리드(8장) / 목록(8장)" 보기를 전환할 수 있습니다(움직이는 WebP는 크게 보기에서 재생).'),
  H3('방법 A — 프로젝트 폴더에서 일괄 업데이트'),
  OL([
    'public/photos/여행/ 등 카테고리 폴더에 이미지 파일을 복사해 넣기(또는 삭제)',
    '새 카테고리: public/photos/새이름/ 폴더를 새로 만들고 이미지를 넣으면 됨',
    '커밋 전에 한 번: npm run optimize:photos 실행 (사진 용량 자동 최적화 — 아래 "사진 용량 최적화" 참고)',
  ...
  UL([
    ...
    '원본은 그대로 저장되므로, 올리기 전 npm run optimize:photos 로 최적화하면 저장소·사이트 용량이 크게 줄어듭니다(아래 참고).',
    '기본 카테고리: 여행, 정보, ZARD, 취미, 개인',
  ]),
  H3('사진 용량 최적화 (중요 — 무료 한도 유지)'),
  P('사진을 추가한 뒤에는 커밋 전에 최적화 스크립트를 한 번 실행하세요. 큰 원본이 그대로 쌓이면 저장소 용량이 빠르게 커지고, GitHub Pages 무료 한도(공개 사이트 권장 1GB)에 가까워집니다.'),
  OL([
    '프로젝트 폴더에서: npm run optimize:photos',
    '정지 이미지: 최대 1600px로 자동 리사이즈·재압축 (이미 작은 파일은 건너뜀, 반복 실행해도 안전)',
    '움직이는 GIF: 애니메이션 WebP(.webp)로 자동 변환 — 용량이 약 10배 이상 줄고 움직임은 그대로 유지(확장자만 .webp로 바뀜)',
    '그 다음 git add → commit → push',
  ]),
  NOTE('관리자 페이지 업로드(아래 방법 B)에는 자동 최적화가 적용되지 않습니다. 사진을 여러 장 올릴 때는 폴더 방식(방법 A)으로 넣고 npm run optimize:photos 를 돌린 뒤 push 하는 것을 권장합니다.'),
  SHOT('photos', '사진 관리 화면 (카테고리별 썸네일 + 업로드/삭제)'),

  H2('5-5. 자료실 (Files)'),
  P('자료실은 사진과 같은 "폴더 방식"입니다. public/files/{카테고리}/ 폴더가 원본이며, 폴더에 파일을 넣거나 지우면 빌드 시 목록이 자동 생성됩니다. 제목=파일명, 유형·크기·날짜는 자동입니다. 방문자 화면의 카테고리 탭에는 자료 건수가 표시됩니다.'),
  P('자료실 메뉴 자체는 누구나 볼 수 있지만, 내용을 열람하려면 "접근 코드"를 입력해야 합니다(민감하지 않은 자료용 간단한 문턱). 관리자로 로그인한 상태면 코드 없이 바로 보입니다.'),
  UL([
    '접근 코드는 환경변수 VITE_FILES_ACCESS_CODE 로 관리합니다: 로컬은 .env 파일, 배포 사이트는 GitHub 저장소 Settings → Secrets and variables → Actions 에 등록.',
    '두 값을 동일하게 맞춰야 로컬·배포 사이트에서 같은 코드로 열립니다.',
    '코드를 바꾸면 재배포가 필요하고, 방문자가 이전에 입력해 통과했던 기록은 자동으로 무효화됩니다(다시 입력해야 함).',
  ]),
  ...
  OL([
    ...
    '자료 삭제: 각 파일의 휴지통 아이콘 → 확인창에서 최종 확인',
    '카테고리 추가: 이름 입력 → "카테고리 추가" → 확인창에서 최종 확인',
    '카테고리 삭제: 카테고리 제목 옆 "카테고리 삭제" 버튼 → 확인창에서 최종 확인',
  ]),
  UL([
    'PDF는 사이트에서 "보기"로 미리보기가 됩니다(그 외 형식은 다운로드).',
    '제목은 파일명에서 자동 생성됩니다. 제목을 바꾸려면 파일명을 바꿔서 올리세요.',
  ]),
  NOTE('카테고리 삭제는 그 안의 자료 전체를 함께 지우며 되돌릴 수 없습니다. 실수 방지를 위해 카테고리 추가·삭제, 자료 삭제 모두 확인창이 뜬 뒤에만 진행됩니다.'),
  ...
  NOTE('관리자 페이지 자체의 기능(코드)이 새로 배포된 경우, 이미 열어 둔 관리자 탭은 그 사실을 모르고 접속 당시의 예전 코드를 계속 씁니다(싱글 페이지 앱이라 자동으로 새로고침되지 않음). 배포 완료 후에도 방금 고친 동작이 안 보이면, 콘텐츠 반영 지연이 아니라 탭이 예전 코드를 쓰고 있는 것일 수 있습니다 — 관리자 탭을 Ctrl+Shift+R 로 새로고침(또는 닫았다 다시 열기)한 뒤 다시 시도하세요.'),
  ...
  UL([
    ...
    '사진 용량 관리: 사진을 추가하면 커밋 전 npm run optimize:photos 실행(정지 이미지 재압축 + GIF→애니메이션 WebP). 안 돌리면 저장소가 빠르게 커집니다.',
    '큰 자료 파일 주의: 자료실 파일도 원본이 그대로 쌓이므로 너무 큰 파일은 줄여서 업로드(파일당 100MB 초과 금지).',
  ...
  H3('업로드한 자료가 방금 고친 대로 안 나와요 (예: 파일명 처리 방식)'),
  UL([
    '관리자에서 파일명 정리 방식 같은 "동작"을 고쳐도, 그 순간 관리자 탭이 이미 열려 있었다면 배포가 끝난 뒤에도 탭은 접속 당시의 예전 코드를 계속 씁니다.',
    '증상 예: 파일명의 공백을 _(밑줄)로 바꾸도록 고쳤는데, 계속 -(하이픈)으로 저장됨 → 배포가 안 된 게 아니라 탭이 새 코드를 못 받아온 것.',
    '해결: Actions 탭에서 배포가 완료됐는지 먼저 확인 → 관리자 탭을 Ctrl+Shift+R 로 강력 새로고침(또는 탭을 닫고 /admin 다시 열기) → 다시 업로드.',
    '이미 잘못된 이름으로 올라간 파일은 자동으로 이름이 바뀌지 않습니다 — 삭제 후 새로고침된 탭에서 다시 업로드하세요.',
  ]),
  ...
  H3('관리자 페이지에서 저장했는데 로컬 프로젝트 폴더에는 안 보여요'),
  UL([
    '관리자(웹) 저장은 GitHub 원격 저장소에 즉시 커밋되지만, 로컬 프로젝트 폴더에는 자동으로 내려오지 않습니다.',
    '터미널에서 프로젝트 폴더로 이동한 뒤 git pull 을 실행하면 최신 내용이 로컬에 반영됩니다.',
  ]),
  H3('로컬 미리보기(localhost)에 새로 올린 사진·자료가 안 보여요'),
  UL([
    'public/photos, public/files 폴더 스캔(목록 생성)은 npm run dev 서버가 시작될 때 한 번만 실행됩니다.',
    '서버가 켜진 상태에서 폴더 내용이 바뀌면(관리자 업로드 후 git pull 등) 화면에 바로 반영되지 않습니다.',
    '터미널에서 Ctrl+C 로 서버를 끄고 npm run dev 로 다시 시작하면 최신 목록이 반영됩니다.',
  ]),
  H3('토큰을 다시 발급해야 하나요?'),
  UL([
    '이미 발급받은 토큰이 아직 유효하면(만료 전 · my-website 저장소에 Contents 쓰기 권한 · 소유자 계정) 그대로 다시 써도 됩니다.',
    '발급된 토큰 목록과 만료일 확인: https://github.com/settings/personal-access-tokens (토큰 값 자체는 발급 시 한 번만 보이고 다시 볼 수 없습니다)',
    '값을 잃어버렸거나 만료됐다면 2장 "사전 준비"의 절차대로 새로 발급하세요.',
  ]),
  ...
  TABLE(['하고 싶은 것', '어디서', '어떻게'], [
    ...
    ['사진 올리기', '/admin → 사진 또는 폴더', '업로드 / 폴더는 push 전 npm run optimize:photos'],
    ['자료 올리기', '/admin → 자료실', '자료 추가 → 파일 업로드 → 저장'],
    ['자료 카테고리 삭제', '/admin → 자료실', '카테고리 옆 "카테고리 삭제" → 확인'],
    ['배포 확인', 'GitHub', 'Actions 탭'],
    ['로컬 최신화', '터미널', 'git pull (안 보이면 npm run dev 재시작)'],
  ]),
